...
export async function clearProtocolCacheById(protocolId: string) {
  ...
  if (API2_SERVER_URL.includes(",")) API2_SERVER_URL = API2_SERVER_URL.split(",");
  else API2_SERVER_URL = [API2_SERVER_URL];

  for (const url of API2_SERVER_URL) {
    const baseUrl = url.endsWith("/") ? url : `${url}/`;
    const endpoint = new URL(`debug-pg/${pgCaceId.replace(/^\/+/, "")}`, baseUrl).toString();
    await axios
      .delete(endpoint, {
        headers: {
